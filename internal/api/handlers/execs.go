...
// PATCH /execs
func PathExecsHandler(w http.ResponseWriter, r *http.Request) {
...
func ResetPasswordHandler(w http.ResponseWriter, r *http.Request) {
	...
	err := json.NewDecoder(r.Body).Decode(&req)
	if err != nil {
		http.Error(w, `{"message":"Invalid values in request"}`, http.StatusBadRequest)
		return
	}

	// ✅ Data Validation for blank values
	if strings.TrimSpace(req.NewPassword) == "" || strings.TrimSpace(req.ConfirmPassword) == "" {
		http.Error(w, `{"message":"Password fields cannot be blank"}`, http.StatusBadRequest)
		return
	}

	if len(req.NewPassword) < 8 {
		http.Error(w, `{"message":"Password must be at least 8 characters long"}`, http.StatusBadRequest)
		return
	}

	var hasUpper, hasDigit, hasSpecial bool = false, false, false
	...
	if !hasUpper || !hasDigit || !hasSpecial {
		http.Error(w, `{"message":"Password must contain at least one uppercase letter, one number, and one special character"}`, http.StatusBadRequest)
		return
	}

	if req.NewPassword != req.ConfirmPassword {
		http.Error(w, `{"message":"Passwords should match}`, http.StatusBadRequest)
		return
	}
	...
	json.NewEncoder(w).Encode(response{Message: "Password reset successfully"})
}
